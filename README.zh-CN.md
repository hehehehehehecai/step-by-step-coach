...
以下操作必须先得到明确确认：force push、重写历史、破坏性重置、删除分支、合并 PR、发布或提升到 Production、回滚、覆盖或删除 Production 环境变量、可能影响用户的重新部署、DNS 修改、部署保护以及 Team/项目访问控制修改。每次确认只授权一个紧接着要执行的高风险动作；其他同时提出的风险操作仍为待处理，必须分别确认。确认轮会说明准备做什么、影响对象、可能不可逆后果和更安全的替代方案，但不会包含可直接执行的生产操作。
