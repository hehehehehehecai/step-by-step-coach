...
Vercel 子 Skill 只处理既有代码周边的部署与配置：从 GitHub、GitLab 或 Bitbucket 导入仓库，构建设置，Preview、Production 与符合当前页面条件的 Custom Environments，提升或回滚部署，环境变量与密钥，Functions，域名、DNS 与 SSL，日志、排错、部署保护以及 Team 或项目访问。
...
以下操作必须先得到明确确认：force push、重写历史、破坏性重置、删除分支、合并 PR、发布或提升到 Production、回滚、删除或覆盖任何环境的变量、改变环境变量作用域、可能影响用户的重新部署、DNS 修改、部署保护以及 Team/项目访问控制修改。每次确认只授权一个紧接着要执行的高风险动作；其他同时提出的风险操作仍为待处理，必须分别确认。确认轮会说明准备做什么、影响对象、可能不可逆后果和更安全的替代方案，但不会包含可直接执行的生产操作。
